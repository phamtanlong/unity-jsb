...
namespace MobileConsole
{
	...
	public class SearchGameObjectViewBuilder : ViewBuilder
	{
		...
		const string Hex_FoundColor = "#54BDFFCC";
		...
		void SearchGameObjects()
		{
			...
			{
				StringComparison comparison = _searchData.ignoreCase ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal;
		...
		static string GetFoundSearchText(string text, SearchData searchData)
		{
			...
			StringComparison comparison = searchData.ignoreCase ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal;
			int startFoundIndex = text.IndexOf(searchData.keyword, comparison);
			if (startFoundIndex < 0)
				return text;
			int endFoundIndex = startFoundIndex + searchData.keyword.Length;
			return string.Format("{0}<color=#54BDFFCC>{1}</color>{2}", 
									text.Substring(0, startFoundIndex),
									text.Substring(startFoundIndex, searchData.keyword.Length),
									text.Substring(endFoundIndex));
		}
	}
}