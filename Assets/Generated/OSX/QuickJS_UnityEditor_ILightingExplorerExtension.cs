...
namespace jsb {
    ...
    // Type: UnityEditor.ILightingExplorerExtension
    [JSBindingAttribute]
    public class QuickJS_UnityEditor_ILightingExplorerExtension
    {
        ...
        public static JSValue Bind_GetContentTabs(JSContext ctx, JSValue this_obj, int argc, JSValue[] argv)
        {
            try
            {
                if (argc == 0)
                {
                    UnityEditor.ILightingExplorerExtension self;
        ...
        {
            var cls = register.CreateClass("ILightingExplorerExtension", typeof(UnityEditor.ILightingExplorerExtension), QuickJS.JSNative.class_private_ctor);
