...
namespace jsb {
    ...
    // Type: Example.New.World.Galaxy
    [JSBindingAttribute]
    public class QuickJS_Example_New_World_Galaxy
    {
        ...
        public static JSValue BindConstructor(JSContext ctx, JSValue new_target, int argc, JSValue[] argv, int magic)
        {
            try
            {
                if (argc == 0)
                {
                    var o = new Example.New.World.Galaxy();
        ...
        public static JSValue BindStatic_GetName(JSContext ctx, JSValue this_obj, int argc, JSValue[] argv)
        {
            try
            {
                if (argc == 0)
                {
                    var ret = Example.New.World.Galaxy.GetName();
        ...
        {
            var cls = register.CreateClass("Galaxy", typeof(Example.New.World.Galaxy), BindConstructor);
