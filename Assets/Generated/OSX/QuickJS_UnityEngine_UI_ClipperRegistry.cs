...
namespace jsb {
    ...
    public class QuickJS_UnityEngine_UI_ClipperRegistry
    {
        ...
        [MonoPInvokeCallbackAttribute(typeof(QuickJS.Native.JSCFunction))]
        public static JSValue BindStatic_Disable(JSContext ctx, JSValue this_obj, int argc, JSValue[] argv)
        {
            try
            {
                if (argc == 1)
                {
                    ...
                    if (!Values.js_get_classvalue(ctx, argv[0], out arg0))
                    {
                        throw new ParameterException(typeof(UnityEngine.UI.ClipperRegistry), "Disable", typeof(UnityEngine.UI.IClipper), 0);
                    }
                    UnityEngine.UI.ClipperRegistry.Disable(arg0);
                    return JSApi.JS_UNDEFINED;
                }
                throw new NoSuitableMethodException("Disable", argc);
            }
            catch (Exception exception)
            {
                return JSNative.ThrowException(ctx, exception);
        ...
        public static QuickJS.Binding.ClassDecl Bind(QuickJS.Binding.TypeRegister register)
        {
            ...
            cls.AddMethod(true, "Disable", BindStatic_Disable);
